<div align="center">
  <h1>Olá, eu sou o Hugo 👋</h1>
  <p><strong>Desenvolvedor Full Stack | Blazor • React • .NET</strong></p>

  <p>
    <a href="https://www.linkedin.com/in/hugo-santos-dev/">
      <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn" />
    </a>
    <a href="https://github.com/hugh514">
      <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
    </a>
    <a href="https://www.instagram.com/hugh_514">
      <img src="https://img.shields.io/badge/Instagram-E4405F?style=flat-square&logo=instagram&logoColor=white" alt="Instagram" />
    </a>
  </p>
</div>

---

### 🧠 Sobre Mim

Sou um desenvolvedor apaixonado por criar soluções robustas e interfaces intuitivas. Atualmente, foco no ecossistema **.NET (Blazor)** e no universo **JavaScript (React & Next.js)**, unindo a performance do C# com a agilidade do front-end moderno.

* 🚀 Atualmente aprimorando conhecimentos em arquitetura Full Stack.
* 📚 Focado em entregar código limpo, escalável e de fácil manutenção.
* 💬 Vamos conversar sobre tecnologia? Me chama em uma das redes acima!

---

### 🛠️ Tecnologias & Ferramentas

| Categoria | Stack |
| :--- | :--- |
| **Linguagens** | C#, JavaScript, TypeScript, SQL |
| **Front-end** | Blazor, React, Next.js, HTML5, CSS3 |
| **Back-end** | .NET Core, Web API, Node.js |
| **DevOps/Tools** | Git, GitHub, MySQL, Vercel |

---

### 🚀 Projeto em Destaque

> **[SigaApp](https://github.com/hugh514/siga-app-blazor)** - Sistema acadêmico completo desenvolvido com Blazor WebAssembly e integração eficiente com banco de dados MySQL. Um exemplo prático de aplicação SPA (Single Page Application) utilizando C#.

---

### 📊 Estatísticas e Atividade

<div align="center">
  <img height="165em" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=hugh514&theme=material_palenight&include_all_commits=true" />
</div>

<br />

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/hugh514/hugh514/output/github-contribution-grid-snake.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/hugh514/hugh514/output/github-contribution-grid-snake.svg" />
    <img alt="Snake animation" src="https://raw.githubusercontent.com/hugh514/hugh514/output/github-contribution-grid-snake.svg" width="80%" />
  </picture>
</div>
